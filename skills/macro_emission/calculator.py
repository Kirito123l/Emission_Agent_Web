...
class MacroEmissionCalculator:
    ...
    def _calculate_link(self, link: Dict, pollutants: List[str],
        ...
        # 获取车队组成
        fleet_mix = link.get("fleet_mix", default_fleet_mix)
        ...
        for pollutant in pollutants:
            ...
            if total_vehicles > 0:
                # 使用 get() 避免 KeyError，当污染物无数据时默认为0
                emission_kg = link_result["total_emissions_kg_per_hr"].get(pollutant, 0)
                rate = (
                    emission_kg * 1000 /
